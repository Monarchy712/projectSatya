...
export default function ContractorRegister() {
    ...
    // Fetch logic yahan handle karinge properly
    useEffect(() => {
        fetchContractors();
    }, []);
    ...
    const handleSubmit = async (e) => {
        ...
        // Validation logic yahan handle karinge
    ...
    return (
        <div className="cr-page">
            <button className="cr-back-home" onClick={() => navigate('/')}>← Back to Home</button>
                ...
                <div className="cr-content">
                    ...
                    {contractors.length > 0 && (
                        <div className="cr-list">
                            <h3 className="cr-list__title">Registered Contractors</h3>
                            <div className="cr-list__items">
                                {contractors.map((c) => (
                                    <div key={c.id} className="cr-list__item">
                                        <div className="cr-list__name">{c.company_name}</div>
                                        <div className="cr-list__wallet">{c.wallet_address.slice(0, 10)}...</div>
                                    </div>
                                ))}
                            </div>
                        </div>
